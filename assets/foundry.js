...
})({"f21vg":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "2e8a4d24ff977811";
"use strict";
...
},{}],"lE7XB":[function(require,module,exports,__globalThis) {
var _arrive = require("arrive");
var _smoothScroll = require("./js/part/smooth-scroll");
document.addEventListener('DOMContentLoaded', (event)=>{
    // Initialize Lenis for smooth scrolling
    (0, _smoothScroll.initSmoothScroll)();
});

},{"arrive":"gDLlb","./js/part/smooth-scroll":"guT3K"}],"gDLlb":[function(require,module,exports,__globalThis) {
/*globals jQuery,Window,HTMLElement,HTMLDocument,HTMLCollection,NodeList,MutationObserver */ /*exported Arrive*/ /*jshint latedef:false */ /*
 * arrive.js
 * v2.5.2
 ...
 */ var Arrive = function(window1, $, undefined1) {
    "use strict";
    if (!window1.MutationObserver || typeof HTMLElement === 'undefined') return; //for unsupported browsers
    ...
    var utils = function() {
        ...
        return {
            ...
            callCallbacks: function(callbacksToBeCalled, registrationData, mutationEvents) {
                ...
                if (registrationData && registrationData.callback && mutationEvents) mutationEvents.addTimeoutHandler(registrationData.target, registrationData.selector, registrationData.callback, registrationData.options, registrationData.data);
            },
            // traverse through all descendants of a node to check if event should be fired for any descendant
            checkChildNodesRecursively: function(nodes, registrationData, matchFunc, callbacksToBeCalled) {
                // check each new node if it matches the selector
                for(var i = 0, node; node = nodes[i]; i++){
                    if (matchFunc(node, registrationData, callbacksToBeCalled)) callbacksToBeCalled.push({
                        callback: registrationData.callback,
                        elem: node
                    });
                    if (node.childNodes.length > 0) utils.checkChildNodesRecursively(node.childNodes, registrationData, matchFunc, callbacksToBeCalled);
                }
            },
            mergeArrays: function(firstArr, secondArr) {
                // Overwrites default options with user-defined options.
                var options = {}, attrName;
                for(attrName in firstArr)if (firstArr.hasOwnProperty(attrName)) options[attrName] = firstArr[attrName];
            ...
            toElementsArray: function(elements) {
                ...
                if (typeof elements !== "undefined" && (typeof elements.length !== "number" || elements === window1)) elements = [
                    elements
                ];
                return elements;
            }
    ...
    var EventsBucket = function() {
        ...
        EventsBucket.prototype.addEvent = function(target, selector, options, callback, data) {
            ...
            if (this._beforeAdding) this._beforeAdding(newEvent);
            this._eventsBucket.push(newEvent);
            return newEvent;
        };
        EventsBucket.prototype.removeEvent = function(compareFunction) {
            for(var i = this._eventsBucket.length - 1, registeredEvent; registeredEvent = this._eventsBucket[i]; i--)if (compareFunction(registeredEvent)) {
                if (this._beforeRemoving) this._beforeRemoving(registeredEvent);
                if (registeredEvent.data && registeredEvent.data.timeoutId) clearTimeout(registeredEvent.data.timeoutId);
                // mark callback as null so that even if an event mutation was already triggered it does not call callback
                var removedEvents = this._eventsBucket.splice(i, 1);
                if (removedEvents && removedEvents.length) removedEvents[0].callback = null;
            }
        };
   ...
   */ var MutationEvents = function(getObserverConfig, onMutation) {
        ...
        eventsBucket.beforeAdding(function(registrationData) {
            ...
            if (target === window1.document || target === window1) target = document.getElementsByTagName("html")[0];
        ...
        this.unbindEvent = function() {
            ...
            eventsBucket.removeEvent(function(eventObj) {
                for(var i = 0; i < elements.length; i++){
                    if (this === undefined1 || eventObj.target === elements[i]) return true;
                }
                return false;
            });
        };
        this.unbindEventWithSelectorOrCallback = function(selector) {
            ...
            if (typeof selector === "function") compareFunction = function(eventObj) {
                for(var i = 0; i < elements.length; i++){
                    if ((this === undefined1 || eventObj.target === elements[i]) && eventObj.callback === callback) return true;
                }
                return false;
            };
            else compareFunction = function(eventObj) {
                for(var i = 0; i < elements.length; i++){
                    if ((this === undefined1 || eventObj.target === elements[i]) && eventObj.selector === selector) return true;
                }
                return false;
            };
        ...
        this.unbindEventWithSelectorAndCallback = function(selector, callback) {
            ...
            eventsBucket.removeEvent(function(eventObj) {
                for(var i = 0; i < elements.length; i++){
                    if ((this === undefined1 || eventObj.target === elements[i]) && eventObj.selector === selector && eventObj.callback === callback) return true;
                }
                return false;
            });
        };
   ...
   */ var ArriveEvents = function() {
        ...
        function getArriveObserverConfig(options) {
            ...
            if (options.fireOnAttributesModification) config.attributes = true;
            return config;
        }
        function onArriveMutation(mutations, registrationData) {
            mutations.forEach(function(mutation) {
                ...
                if (newNodes !== null && newNodes.length > 0) utils.checkChildNodesRecursively(newNodes, registrationData, nodeMatchFunc, callbacksToBeCalled);
                else if (mutation.type === "attributes") {
                    if (nodeMatchFunc(targetNode, registrationData, callbacksToBeCalled)) callbacksToBeCalled.push({
                        callback: registrationData.callback,
                        elem: targetNode
                    });
                }
        ...
        function nodeMatchFunc(node, registrationData, callbacksToBeCalled) {
            ...
            if (utils.matchesSelector(node, registrationData.selector)) {
                if (node._id === undefined1) node._id = arriveUniqueId++;
        ...
        arriveEvents.bindEvent = function(selector, arg2, arg3) {
            var options = typeof arg2 === 'object' ? utils.mergeArrays(arriveDefaultOptions, arg2) : {
                ...arriveDefaultOptions
            };
            var callback = typeof arg3 === 'function' ? arg3 : typeof arg2 === 'function' ? arg2 : undefined1;
            var elements = utils.toElementsArray(this);
            ...
            if (options.existing) {
                ...
                for(var i = 0; i < elements.length; i++){
                    ...
                    for(var j = 0; j < nodes.length; j++)existing.push({
                        callback: callback,
                        elem: nodes[j]
                    });
                }
                ...
                if (options.onceOnly && existing.length) {
                    if (callback) return callback.call(existing[0].elem, existing[0].elem);
                    else return Promise.resolve(existing[0].elem);
                }
                setTimeout(utils.callCallbacks, 1, existing);
            }
            if (callback) mutationBindEvent.call(this, selector, options, callback);
            else {
                var a = this;
   ...
   */ var LeaveEvents = function() {
        ...
        function onLeaveMutation(mutations, registrationData) {
            mutations.forEach(function(mutation) {
                ...
                if (removedNodes !== null && removedNodes.length > 0) utils.checkChildNodesRecursively(removedNodes, registrationData, nodeMatchFunc, callbacksToBeCalled);
        ...
        leaveEvents.bindEvent = function(selector, arg2, arg3) {
            var options = typeof arg2 === 'object' ? utils.mergeArrays(leaveDefaultOptions, arg2) : {
                ...leaveDefaultOptions
            };
            var callback = typeof arg3 === 'function' ? arg3 : typeof arg2 === 'function' ? arg2 : undefined1;
            if (callback) mutationBindEvent.call(this, selector, options, callback);
    ...
    /*** expose APIs ***/ function exposeApi(exposeTo) {
...
},{}],"guT3K":[function(require,module,exports,__globalThis) {
 ...
 */ parcelHelpers.export(exports, "initSmoothScroll", ()=>initSmoothScroll);
...
},{"lenis":"JS2ak","gsap":"fPSuC","gsap/ScrollTrigger":"7wnFk","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"JS2ak":[function(require,module,exports,__globalThis) {
// package.json
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
    ...
    onTransitionEnd = (event)=>{
        if (event.propertyName.includes("overflow")) {
            ...
            if ([
                "hidden",
                "clip"
            ].includes(overflow)) this.internalStop();
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["f21vg","lE7XB"], "lE7XB", "parcelRequireb0e7", {})
